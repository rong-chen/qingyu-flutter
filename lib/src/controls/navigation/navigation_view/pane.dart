...
class NavigationPane with Diagnosticable {
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties..add(EnumProperty<PaneDisplayMode>(
      'displayMode',
      displayMode,
      ...
      footerItems,
      defaultValue: [],
    ))..add(IntProperty('selected', selected, ifNull: 'none'))..add(
        ObjectFlagProperty('onChanged', onChanged, ifNull: 'disabled'))..add(
        ObjectFlagProperty('onItemPressed', onItemPressed,
            ifNull: 'disabled'))..add(DiagnosticsProperty<ScrollController>(
        'scrollController', scrollController))..add(
        DiagnosticsProperty<NavigationPaneSize>('size', size))..add(
        ObjectFlagProperty<Widget>.has('autoSuggestBox', autoSuggestBox));
  }
  ...
  static Widget buildMenuButton(BuildContext context,
    ...
    if (pane.menuButton != null && pane.Expand) return pane.menuButton!;
    return Container(
...
class NavigationPaneSize with Diagnosticable {
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    ))..add(DoubleProperty('openMinWidth', openMinWidth))..add(
        DoubleProperty('openMaxWidth', openMaxWidth))..add(DoubleProperty(
      'headerHeight',
      headerHeight,
      defaultValue: kOneLineTileHeight,
    ));
...
MenuFlyoutItemBase _buildMenuPaneItem(BuildContext context,
...
class _CompactNavigationPane extends StatelessWidget {
  ...
  static Widget _buildItem(NavigationPaneItem item) {
    return Builder(builder: (context) {
      assert(debugCheckHasFluentTheme(context));
      final pane = InheritedNavigationView
          .of(context)
          .pane!;
      if (item is PaneItemHeader) {
    ...
    return AnimatedContainer(
      ...
      child: Align(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              () {
            if (!pane.Expand) return const SizedBox(
              height: 50,
            );
          ...
          if (showReplacement)
            Padding(
              padding: topPadding,
              child: PaneItem(
                title: Text(FluentLocalizations
                    .of(context)
                    .clickToSearch),
...
class _OpenNavigationPane extends StatefulWidget {
  ...
  static Widget buildItem(NavigationPane pane,
      NavigationPaneItem item, [
